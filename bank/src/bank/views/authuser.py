...
class AuthUser(Resource):
    def get(self, provider=None):
        try:
            ...
            if denied != '':
                return redirect(url_for('.login', denied=denied))
            ...
            if r1.status_code != 200:
                ...
                if r1.status_code != 200:
                    ...
                    if dbuser is not None and len(dbuser) > 0:
                        ...
                        session['provider'] = dbuser[0]['provider_name']  + " / " + username

                        return redirect(url_for('.home'))

                    else:

                        dbuser = db.Select("SELECT id,name,email FROM users WHERE email = %s",( email, ))
                        if dbuser is not None and len(dbuser) > 0:

                            session['id'] = dbuser[0]['id']
                            session['email'] = dbuser[0]['email']
                            session['provider'] = provider_name + " / " + username

                            return redirect(url_for('.home'))

                        else:
                            return resp_error('Usuário local não contrado com o e-mail %s' % email)
